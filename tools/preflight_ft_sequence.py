...
EXPECTED = '6715a014823ac3b7365b8e8d64625486330f706f87191ddcdef2a313f599cbed'

def sha(path):
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()

def main():
    parser=argparse.ArgumentParser();parser.add_argument('strategy',choices=['no_droppath','no_diff','velocity10']);opts=parser.parse_args()
    config=f'configs/pose3d/ft_sequence_{opts.strategy}_8e.yaml'
    set_random_seed(0)
    out = Path('verification') / opts.strategy
    out.mkdir(parents=True,exist_ok=True)
    init = Path(f'initializers/{opts.strategy}/source_ema.bin')
    init.parent.mkdir(parents=True,exist_ok=True)
    assert not init.exists(), 'Never overwrite initializer'
    assert sha(SOURCE) == EXPECTED
    payload = torch.load(SOURCE, map_location='cpu', weights_only=False)
    assert payload['epoch'] == 45 and payload['checkpoint_type'] == 'ema'
    weights = payload['model_pos']
    assert all(torch.isfinite(v).all() for v in weights.values() if v.is_floating_point())
    c = get_config(config)
    expected={'no_droppath':(0.,.5,20.),'no_diff':(.2,0.,20.),'velocity10':(.2,.5,10.)}[opts.strategy]
    assert (c.drop_path_rate,c.lambda_diff,c.lambda_3d_velocity)==expected
    assert c.dim_feat == 128 and c.depth == 20 and c.epochs == 8
    assert c.batch_size == 4 and c.finetune and not c.gt_2d
    assert c.learning_rate == 1e-6 and c.warmup_epochs == 1 and c.max_grad_norm == 1
    assert c.ema_decay == .9998
    model = load_backbone(c)
    model.load_state_dict(weights, strict=True)
    assert sum(p.numel() for p in model.parameters()) == 6836355
    ...
    report = {'status': 'PENDING', 'strategy':opts.strategy, 'source_checkpoint': str(SOURCE), 'source_sha256': EXPECTED,
    ...
    for batch in (1, 2, 4):
        ...
        assert sum(p.numel() for g in groups for p in g['params']) == 6836355
        assert all(g['weight_decay']==.012 for g in groups)
        groups=layerwise_lr_groups(base,groups,c)
        optimizer = torch.optim.AdamW(groups, lr=c.learning_rate)
        assert [g['lr'] for g in optimizer.param_groups]==[1e-6,1e-6,3e-6,1e-5]
        ...
        assert math.isclose(schedule.scale_at(0) * c.learning_rate, 1e-7)
        ...
        assert all(p.requires_grad and p.grad is not None and torch.isfinite(p.grad).all() for p in base.parameters())
        for prefix in [f'blocks.{i}.' for i in range(20)]+['head.','Spatial_patch_to_embedding.']:
            assert any(not torch.equal(p.detach().cpu(),weights[n]) for n,p in base.named_parameters() if n.startswith(prefix)),prefix
        for n in ('Spatial_pos_embed','Temporal_pos_embed'):
            assert not torch.equal(base.state_dict()[n].cpu(),weights[n]),n
        assert all(block.training for block in base.blocks)
        for group,peak_lr in zip(optimizer.param_groups,schedule.base_lrs):
            assert math.isclose(group['lr'],peak_lr*schedule.scale_at(1))
        ...
        report['stages'][f'B{batch}'] = {'loss':meters['total'].avg,
            ...
            'optimizer_reset':True, 'ema_reset':True, 'compiled': batch == 4}

...
CONFIG = 'configs/pose3d/graph_posemamba_h36m_w256_selective_ft_b2_5e.yaml'
...
def main():
    ...
    c = get_config(CONFIG)
    assert c.dim_feat == 256 and c.depth == 16 and c.epochs == 5
    ...
    assert all(p.requires_grad == n.startswith(('blocks.15.', 'head.')) for n,p in model.named_parameters())
    ...
    for batch in (1, 2):
        ...
        assert all(not m.training for block in base.blocks[:-1] for m in block.modules())
        assert base.blocks[-1].training and base.head.training

...
def main():
    ...
    init = Path('initializers/w128d20_e45_ema.bin')
    ...
    c = get_config(CONFIG)
    assert c.finetune_jitter_std==.002 and c.finetune_jitter_probability==.5
    assert c.dim_feat == 128 and c.depth == 20 and c.epochs == 8
    assert c.batch_size == 4 and c.drop_path_rate == .2 and c.finetune and not c.gt_2d

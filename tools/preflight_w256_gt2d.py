...
def main():
    ...
    init = Path('initializers/gt2d_init_ema.bin')
    ...
    c = get_config(CONFIG)
    assert c.dim_feat == 256 and c.depth == 16 and c.epochs == 30
    assert c.batch_size == 4 and c.drop_path_rate == .2 and c.finetune and c.gt_2d
    assert c.learning_rate == 3e-5 and c.warmup_epochs == 3 and c.max_grad_norm == 1
    ...
    for batch in (1, 2, 4):
        ...
        assert math.isclose(schedule.scale_at(0) * c.learning_rate, 3e-6)

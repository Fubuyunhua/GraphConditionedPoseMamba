...
def main():
    cfg='configs/pose3d/graph_posemamba_h36m_w128_d10_balanced_det2d_80e.yaml'
    c=get_config(cfg)
    assert c.dim_feat==128 and c.depth==10 and c.epochs==80 and c.batch_size==8
    assert not c.gt_2d and not c.finetune and not c.pretrained and not c.resume and not c.evaluate
    ...
    assert abs(c.ema_decay-.9998**2)<1e-12
    out=Path('verification');out.mkdir(exist_ok=True)
    set_random_seed(0)
    initial=load_backbone(c)
    assert sum(p.numel() for p in initial.parameters())==3435395
    ...
    for split in ('train','test'):
        ...
        assert not torch.equal(x[...,:2],y[...,:2]), 'Detector input unexpectedly equals GT'
    c.mask=False
    report={'status':'PENDING','initialization':'random seed0; no checkpoint loading',
            ...
            'parameters':3435395,'parallel_with_pid':2004698,'stages':{}}
    ...
    for batch in (1,2,4,8):
        ...
        assert len(groups)==1 and sum(p.numel() for p in groups[0]['params'])==3435395
        ...
        if batch==8:
            tail_loader=DataLoader(dataset,batch_size=4,shuffle=False,num_workers=0)
            train_epoch(c,model,itertools.islice(tail_loader,1),meters,optimizer,has_3d=True,has_gt=True,ema_helper=ema)
        assert ema.num_updates==(3 if batch==8 else 2)
        ...
        report['stages'][f'B{batch}']={'loss':meters['total'].avg,'grad_norm':meters['grad_norm'].avg,
            'peak_reserved_mib':peak,'roundtrip':True,'random_initialization_verified':True,'tail_batch4_compiled_check':batch==8}
        del model,base,optimizer,groups,ema,state,reloaded,sample,before
        gc.collect();torch.cuda.empty_cache()
    report['status']='PASS'
    (out/'w128_d10_balanced_det2d_preflight.json').write_text(json.dumps(report,indent=2))

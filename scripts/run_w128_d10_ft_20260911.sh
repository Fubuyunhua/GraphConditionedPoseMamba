#!/usr/bin/env bash
set -euo pipefail
cd /scratch/home/caiwei/GraphConditionedPoseMamba_W128_D10_BEST_FT_20260911
...
"$py" -u tools/preflight_w128_d10_ft.py
"$py" -u train.py --config "$config" --checkpoint runs/initial_eval --evaluate initializers/d10_e63_best_ema.bin --seed 0 > verification/initial_eval.log 2>&1
"$py" - <<'PY'
...
assert abs(p1-39.383528)<.02 and abs(p2-32.778690)<.02,(p1,p2)
...
set +e
"$py" -u train.py --config "$config" --checkpoint runs/w128_d10_best_ft_20260911/seed0 --pretrained initializers --selection d10_e63_best_ema.bin --seed 0
code=$?

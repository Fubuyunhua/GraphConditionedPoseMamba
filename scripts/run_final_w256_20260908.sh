...
echo 'Waiting for W128 complete80 and idle GPU'
while kill -0 1407974 2>/dev/null; do sleep 30; done
test "$(cat "$old/verification/formal_exit_code.txt")" = 0
grep -aq '\[80\] time' "$old/launch_logs/final_w128_det2d_seed0.log"
